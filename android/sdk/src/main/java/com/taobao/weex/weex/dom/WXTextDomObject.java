...
public class WXTextDomObject extends WXDomObject {
  ...
  private void createUrlSpan(SpannableString ss,String text) {
    if (!TextUtils.isEmpty(text)) {
      String HTTP_REGX = "(((http|ftp|https|file)://)|((?<!((http|ftp|https|file)://))www\\.))"// 以http...或www开头
          + ".*?"                                                                   // 中间为任意内容，惰性匹配
          + "(?=(&nbsp;|\\s|　|<br />|$|，|,|[\u4e00-\u9fa5]|[<>]))";
      Pattern pattern = Pattern.compile(HTTP_REGX, Pattern.CASE_INSENSITIVE);
      Matcher matcher = pattern.matcher(text);
      int startPoint = 0;
      while (matcher.find(startPoint)) {
        int endPoint = matcher.end();
        String url = matcher.group();
        URLSpan newSpan = new TianyaURLSpan(url, urlClickListener, false);
        if (!url.contains(".jpg") || !url.contains(".gif")) {
          ss.setSpan(newSpan, endPoint - url.length(), endPoint, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        startPoint = endPoint;
      }
    }
  ...
  class TianyaURLSpan extends URLSpan {
    //    public static final int URL_HREF_COLOR = Color.rgb(3, 169, 244);
    public final int URL_HREF_COLOR = Color.rgb(106, 131, 168);

    protected int mUrkHrefColor = URL_HREF_COLOR;
    private OnUrlClickListener listener;
    private final boolean underLine;

    public TianyaURLSpan(String url, OnUrlClickListener listener,boolean underLine) {
      super(url);
    ...
    public void updateDrawState(TextPaint textpaint) {
      ...
      //<color name="username">#6a83a8</color>
      textpaint.setColor(mUrkHrefColor);
      textpaint.setUnderlineText(underLine);// 木有下划线
    }
  ...
  OnUrlClickListener urlClickListener = new OnUrlClickListener() {
    @Override public void onUrlClick(String url) {
      Map<String, Object> map = new HashMap<>();
      map.put("url", url);
      WXSDKManager.getInstance().getSDKInstance(getDomContext().getInstanceId()).fireEvent(getRef(), "urlClick", map);
    }
  };
}
