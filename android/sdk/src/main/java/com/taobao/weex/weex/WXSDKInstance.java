...
public class WXSDKInstance implements IWXActivityStateListener, DomContext, View.OnLayoutChangeListener {
  ...
  private @NonNull FlatGUIContext mFlatGUIContext = new FlatGUIContext();
  ...
  @RestrictTo(Scope.LIBRARY) public @NonNull FlatGUIContext getFlatUIContext() {
  ...
  */
  @Deprecated public void setViewPortWidth(int viewPortWidth) {
   ...
   */
  @Deprecated public static int getViewPortWidth() {
   ...
   * For unittest only.
   */
  @RestrictTo(Scope.TESTS) WXSDKInstance(Context context, String id) {
    mInstanceId = id;
    init(context);
  }
  ...
  public void init(Context context) {
    ...
    mCallbackHeader = new StringBuffer(mInstanceId).append(CALLBACK_SPLIT).toString();
  ...
  @Deprecated public void registerScrollViewListener(WXScrollViewListener scrollViewListener) {
    mWXScrollViewListener = scrollViewListener;
  }

  @Deprecated public WXScrollViewListener getScrollViewListener() {
    return mWXScrollViewListener;
  }

  @Deprecated public void setIWXUserTrackAdapter(IWXUserTrackAdapter adapter) {
  }
   ...
   */
  @Deprecated public void render(String template, Map<String, Object> options, String jsonInitData, WXRenderStrategy flag) {
   ...
   */
  @Deprecated public void render(String pageName, String template, Map<String, Object> options, String jsonInitData, int width, int height, WXRenderStrategy flag) {
   ...
   */
  @Deprecated public void render(String template, int width, int height) {
    render(template);
  }
   ...
   */
  @Deprecated public void renderByUrl(String pageName, final String url, Map<String, Object> options, final String jsonInitData, final int width, final int height, final WXRenderStrategy flag) {
  ...
  public @Nullable IWebSocketAdapter getWXWebSocketAdapter() {
    return WXSDKManager.getInstance().getIWXWebSocketAdapter();
  }

  @Deprecated public void reloadImages() {
    if (mScrollView == null) {
      return;
    }
  }
  ...
  @Deprecated public void registerActivityStateListener(IWXActivityStateListener listener) {
  ...
  @Override public boolean onActivityBack() {
  ...
  @Override public final void onLayoutChange(View v, int left, int top, int right, int bottom, int oldLeft, int oldTop, int oldRight, int oldBottom) {
  ...
  @Deprecated public void setBundleUrl(String url) {
    mBundleUrl = url;
    if (WXSDKManager.getInstance().getValidateProcessor() != null) {
      mNeedValidate = WXSDKManager.getInstance().getValidateProcessor().needValidate(mBundleUrl);
    }
  ...
  /*Global Event*/
  private static HashMap<String, List<String>> mGlobalEvents = new HashMap<>();
  private final String CALLBACK_SPLIT = " ";
  private String mCallbackHeader = "";
  ...
  public void fireAllEventCallback(String eventName, Map<String, Object> params) {
    List<String> callbacks = mGlobalEvents.get(eventName);
    if (callbacks != null) {
      for (String callback : callbacks) {
        String[] array = callback.split(CALLBACK_SPLIT);
        if (array.length == 2) {
          WXSDKManager.getInstance().callback(array[0], array[1], params, true);
        }
      }
    }
  }
  ...
  protected void addEventListener(String eventName, String callback) {
    ...
    callbacks.add(new StringBuffer(mCallbackHeader).append(callback).toString());
  }
  ...
  protected void removeEventListener(String eventName, String callback) {
    ...
    List<String> callbacks = mGlobalEvents.get(eventName);
    if (callbacks != null) {
      callbacks.remove(new StringBuffer(mCallbackHeader).append(callback).toString());
    }
  }
  ...
  protected void removeEventListener(String eventName) {
    ...
    List<String> callbacks = mGlobalEvents.get(eventName);
    if (callbacks != null) {
      Iterator<String> iterator = callbacks.iterator();
      while (iterator.hasNext()) {
        String str = iterator.next();
        if (str != null && str.startsWith(mCallbackHeader)) {
          iterator.remove();
        }
      }
      if (callbacks.isEmpty()) mGlobalEvents.remove(eventName);
    }
  }
  ...
  class WXHttpListener implements IWXHttpAdapter.OnHttpListener {
    ...
    @Override public void onHttpStart() {
      if (this.instance != null && this.instance.getWXStatisticsListener() != null) {
        this.instance.getWXStatisticsListener().onHttpStart();
      }
    }

    @Override public void onHeadersReceived(int statusCode, Map<String, List<String>> headers) {
      if (this.instance != null && this.instance.getWXStatisticsListener() != null) {
    ...
    @Override public void onHttpUploadProgress(int uploadProgress) {

    }

    @Override public void onHttpResponseProgress(int loadedLength) {

    }

    @Override public void onHttpFinish(WXResponse response) {
      if (this.instance != null && this.instance.getWXStatisticsListener() != null) {
      ...
      if (response.extendParams != null) {
        ...
        if ("network".equals(requestType) && mUserTrackAdapter != null) {
          ...
          if (!"200".equals(response.statusCode)) {
            ...
            WXExceptionUtils.commitCriticalExceptionRT(getInstanceId(), WXErrorCode.WX_KEY_EXCEPTION_JS_DOWNLOAD_FAILED.getErrorCode(), "WX_KEY_EXCEPTION_JS_DOWNLOAD_FAILED",
                WXErrorCode.WX_KEY_EXCEPTION_JS_DOWNLOAD_FAILED.getErrorMsg() + "\n response.errorCode=" + response.errorCode + "\n response.errorMsg=" + response.errorMsg + "\n response=" + getTemplateInfo(), null);
            ...
            WXExceptionUtils.commitCriticalExceptionRT(getInstanceId(), WXErrorCode.WX_KEY_EXCEPTION_JS_DOWNLOAD_FAILED.getErrorCode(), "WX_KEY_EXCEPTION_JS_DOWNLOAD_FAILED_TEMPLATE_NULL",
                WXErrorCode.WX_KEY_EXCEPTION_JS_DOWNLOAD_FAILED.getErrorMsg() + "\n response.errorCode=" + response.errorCode + "\n response.errorMsg=" + response.errorMsg + "\n response=" + getTemplateInfo(), null);
          } else {
            performance.errCode = WXErrorCode.WX_SUCCESS.getErrorCode();
          }
      ...
      if (response != null && response.originalData != null && TextUtils.equals("200", response.statusCode)) {
        ...
        // check content-type
      } else if (TextUtils.equals(WXRenderErrorCode.DegradPassivityCode.WX_DEGRAD_ERR_BUNDLE_CONTENTTYPE_ERROR.getDegradErrorCode(), response.statusCode)) {
        ...
        onRenderError(WXRenderErrorCode.DegradPassivityCode.WX_DEGRAD_ERR_BUNDLE_CONTENTTYPE_ERROR.getDegradErrorCode(),
            "|response.errorMsg==" + response.errorMsg + "|instance.getTemplateInfo == \n" + instance.getTemplateInfo() + "|instance bundleUrl = \n" + instance.getBundleUrl() + "|instance requestUrl = \n" + Uri.decode(WXSDKInstance.requestUrl));
        ...
        onRenderError(WXRenderErrorCode.DegradPassivityCode.WX_DEGRAD_ERR_NETWORK_CHECK_CONTENT_LENGTH_FAILED.getDegradErrorCode(),
            WXRenderErrorCode.DegradPassivityCode.WX_DEGRAD_ERR_NETWORK_CHECK_CONTENT_LENGTH_FAILED.getDegradErrorMsg() + "|response.errorMsg==" + response.errorMsg + "|instance.getTemplateInfo == \n" + instance.getTemplateInfo());
      } else {
        onRenderError(WXRenderErrorCode.DegradPassivityCode.WX_DEGRAD_ERR_NETWORK_BUNDLE_DOWNLOAD_FAILED.getDegradErrorCode(), response.errorMsg);
      }
    }
  }
  ...
  public String getTemplateInfo() {
    ...
    try {
      ...
      return " template md5 " + WXFileUtils.md5(bts) + " length " + bts.length + "response header " + JSONObject.toJSONString(responseHeaders);
