...
public abstract class WXComponent<T extends View> implements IWXObject, IWXActivityStateListener, OnActivePseudoListener {
  ...
  @Deprecated public WXComponent(WXSDKInstance instance, WXDomObject dom, WXVContainer parent, String instanceId, boolean isLazy) {
    this(instance, dom, parent, isLazy);
  }

  @Deprecated public WXComponent(WXSDKInstance instance, WXDomObject dom, WXVContainer parent, boolean isLazy) {
  ...
  public final void setLayout(ImmutableDomObject domObject) {
    ...
    int realLeft, realTop, realRight, realBottom;
    if (!nullParent && (mParent.getHostView() instanceof LinearLayout || mParent.getHostView() instanceof RelativeLayout || mParent.getHostView() instanceof CoordinatorLayout)) {
      WXStyle mStyle = mDomObj.getStyles();
      realLeft = (int) mStyle.getMarginLeft();
      realRight = (int) mStyle.getMarginRight();
      realTop = (int) mStyle.getMarginTop();
      realBottom = (int) mStyle.getMarginBottom();
    } else {
      realLeft = (int) (mDomObj.getLayoutX() - parentPadding.get(Spacing.LEFT) - parentBorder.get(Spacing.LEFT));
      realTop = ((int) (mDomObj.getLayoutY() - parentPadding.get(Spacing.TOP) - parentBorder.get(Spacing.TOP)) + siblingOffset);
      realRight = (int) margin.get(Spacing.RIGHT);
      realBottom = (int) margin.get(Spacing.BOTTOM);
    }
  ...
  @Deprecated public void updateProperties(Map<String, Object> props) {
  ...
  protected boolean setProperty(String key, Object param) {
    switch (key) {
      ...
      case Constants.Name.POSITION:
        ...
        if (position != null) setSticky(position);
        return true;
      ...
      case Constants.Name.BORDER_LEFT_WIDTH:
        ...
        if (width != null) setBorderWidth(key, width);
        return true;
      ...
      case Constants.Name.BORDER_TOP_STYLE:
        ...
        if (border_style != null) setBorderStyle(key, border_style);
        return true;
      ...
      case Constants.Name.BORDER_LEFT_COLOR:
        ...
        if (border_color != null) setBorderColor(key, border_color);
        return true;
      case Constants.Name.VISIBILITY:
        String visibility = WXUtils.getString(param, null);
        if (visibility != null) setVisibility(visibility);
        return true;
  ...
  @TargetApi(Build.VERSION_CODES.JELLY_BEAN) protected void setAriaHidden(boolean isHidden) {
  ...
  public void addEvent(String type) {
    if (TextUtils.isEmpty(type) || mAppendEvents.contains(type) || getRealView() == null) {
      return;
    }
   ...
   * get Scroller components
   */
  @Nullable public Scrollable getFirstScroller() {
   ...
   */
  @Deprecated protected void initView() {
    if (mContext != null) mHost = initComponentHostView(mContext);
  }
   ...
   */
  @CallSuper protected void onHostViewInitialized(T host) {
   ...
   */
  @Deprecated public View getView() {
    return mHost;
  }
  ...
  @Deprecated public void registerActivityStateListener() {
   ...
   ********************************************************/
  @CallSuper public void recycled() {
    if (mDomObj.isFixed()) return;
   ...
   */
  @CheckResult protected Object convertEmptyProperty(String propName, Object originalValue) {
    switch (propName) {
      case Constants.Name.BACKGROUND_COLOR:
        return "transparent";
      case Constants.Name.BORDER_RADIUS:
  ...
  @Override public void updateActivePseudo(boolean isSet) {
  ...
  @IntDef({ STATE_DOM_FINISH, STATE_UI_FINISH, STATE_ALL_FINISH }) @Retention(RetentionPolicy.SOURCE) @Target(ElementType.PARAMETER) public @interface RenderState {
  }

  @CallSuper public void onRenderFinish(@RenderState int state) {
